...
/*
 * 思路：
 * 类似于N皇后，也是遍历每个位置，每个位置分别遍历1-9能不能放
 * 找到能放的数字后，先放入，然后遍历下一个位置
 * 如果1-9都用过了，说明前面有位置放错了数字，回溯
*/
...
public:
    ...
    void dfs(vector<vector<char>>& board, int pos) {
        ...
        //! 即mask某位为1，说明该位对应的数字可以放在该空格
        int mask = (~(line[i] | column[j] | block[i / 3][j / 3])) & 0x1ff;
        //* mask & (mask - 1)把mask最低位的1变为0，则该位对应数字不再可用
        for (; mask && !valid; mask &= (mask - 1)) {
            //* 这个操作只保留最低位的1
            int digitMask = mask & (-mask);
            //* 最低位的1后面的位数也就是最低位的1在第几位上（digit数值范围为0-8）
    ...
    void solveSudoku(vector<vector<char>>& board) {
        ...
        //* 如果一个空白格只有唯一的数可以填入，
        //* 也就是其对应的 mask 值和 mask−1 进行按位与运算后得到 0（即 b 中只有一个二进制位为 1）。
