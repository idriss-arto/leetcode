...
/*
 * 我的思路：
 * 让end向右滑动，直到遇到不等于first或second的水果
 * （滑动过程中更新result，如果写成遇到第三种水果再更新，在只有两种水果时会返回0，如[0,1,0]）
 * end指向第三种水果后，second = fruits[end]
 * 但是first不能直接取second，要从end位置向前遍历，直到遇到第一个区别于second的水果，否则first可能取错
 ! 后续思考后，其实此时end和end-1一定不是同一种水果
 * 如[1,0,1,4,1,4,1,2,3]，当end指向第一个4时，second为0，要更新为4。first为1，要更新为1。
 * first确定取值后，要向前遍历确定begin位置。
 * 
 * 题解思路：
 * 用unordered_map<int, int> cnt来记录水果品种和对应水果数量
 * 当cnt.size()>2时，说明出现第三种水果，
 * 此时left指向的水果数量减一，然后left右移
 * 直到left指向的水果数量减一后数量为0
*/
    ...
    int totalFruit(vector<int>& fruits) {
        ...
        for(; end < fruits.size(); end++)
        {
            ...
            //* 新水果，判断第二个篮子是否为空
            {
                ...
                {
                    second = fruits[end];       
                    begin = end - 1;            //* 此时end和end-1一定不是同一种水果
                    first = fruits[begin];
